...
You can enable tab completion including known URI's when using bash. This requires
using bash 4.4 or newer. To do this you need to source the `.hab-complete.bash`
file. This is installed next to the `hab` script (This makes its path consistent
even with editable installs). Use `which .hab-complete.bash` to locate the file.
If that doesn't work it should be next to the file returned by `which hab`.
